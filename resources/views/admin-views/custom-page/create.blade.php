...
@section('title', translate('messages.add_custom_page'))

@push('css_or_js')
<style>
/* ── Picker shared ─────────────────────────────────────────── */
.cp-section-card { border: 1px solid #e7eaf3; border-radius: 10px; overflow: hidden; }
.cp-section-card .cp-section-header {
...
.cp-grid-item {
    border: 2px solid #e7eaf3; border-radius: 8px; cursor: pointer;
...
.cp-grid-item.selected::after {
    content: '\2713'; position: absolute; top: 5px; right: 7px;
    background: #FC6A57; color: #fff; border-radius: 50%;
    width: 18px; height: 18px; font-size: 11px; line-height: 18px; text-align: center;
}
.cp-grid-item img {
    width: 72px; height: 72px; object-fit: cover; border-radius: 8px; margin-bottom: 7px;
}
...
.cp-selected-item {
    ...
    padding: 5px 10px 5px 6px; font-size: 12px; color: #1e2022;
}
...
<div class="content container-fluid">
        ...
        <div class="row align-items-center">
            <div class="col-sm mb-2 mb-sm-0">
                <h1 class="page-header-title">
                    <span class="page-header-icon"><i class="tio-add-circle-outlined text-primary"></i></span>
                    {{ translate('messages.add_custom_page') }}
                </h1>
            </div>
    ...
    <form id="custom-page-form" action="{{ route('admin.custom-page.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
            {{-- LEFT --}}
            <div class="col-lg-8">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                <input type="text" name="title" class="form-control" placeholder="e.g. Summer Specials" required>
                            </div>
                            <div class="col-md-6 form-group">
                                <label class="input-label">Subtitle</label>
                                <input type="text" name="subtitle" class="form-control" placeholder="e.g. Best deals this week">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="input-label">Promotional Text</label>
                            <textarea name="promotional_text" class="form-control" rows="2" placeholder="e.g. Limited time offer!"></textarea>
                        </div>
                        <div class="form-group mb-0">
                            <label class="input-label">Background Color</label>
                            <div class="d-flex align-items-center">
                                <input type="color" name="background_color" id="bg_color_picker" value="#ffffff"
                                       style="width:48px;height:38px;padding:2px;border-radius:6px;border:1px solid #e7eaf3;cursor:pointer;">
                                <input type="text" id="bg_color_hex" value="#ffffff" maxlength="7"
                                       class="form-control ml-2" style="width:110px;" placeholder="#ffffff">
                                <small class="text-muted ml-2">Click swatch or type hex</small>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="cp-section-card mb-3">
                    <div class="cp-section-header">
                        ...
                        <div class="cp-toolbar">
                            ...
                            <button type="button" class="btn btn-sm btn-white" id="product-search-clear" title="Clear search">
                                <i class="tio-clear"></i>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="cp-selected-area">
                        ...
                        <div class="cp-selected-list" id="product-selected-list">
                            <span class="cp-empty-msg">No products added yet — click items above to add</span>
                        </div>
                ...
                <div class="cp-section-card mb-3">
                    <div class="cp-section-header">
                        ...
                        <div class="cp-toolbar">
                            ...
                            <button type="button" class="btn btn-sm btn-white" id="restaurant-search-clear" title="Clear search">
                                <i class="tio-clear"></i>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="cp-selected-area">
                        ...
                        <div class="cp-selected-list" id="restaurant-selected-list">
                            <span class="cp-empty-msg">No restaurants added yet — click items above to add</span>
                        </div>
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body text-center">
                        <img id="bg-image-preview"
                             src="{{ dynamicAsset('public/assets/admin/img/900x400/img1.jpg') }}"
                        ...
                        <label class="btn btn-sm btn-outline-primary btn-block" for="background_image">
                            <i class="tio-upload mr-1"></i> Upload Image
                        </label>
                ...
                <div class="card">
                    <div class="card-body">
                        <button type="submit" class="btn btn-primary btn-block" id="save-btn">
                            <i class="tio-save mr-1"></i> Save Page
                        </button>
...
function GridPicker(cfg) {
    ...
    function renderGrid(items) {
        ...
        items.forEach(item => {
            ...
            const price   = item.price !== undefined ? '<div class="cp-item-price">' + cfg.currency + parseFloat(item.price).toFixed(2) + '</div>' : '';
            div.innerHTML =
                '<img src="' + imgSrc + '" onerror="this.src=\'' + cfg.fallback + '\'" alt="">' +
                '<div class="cp-item-name">' + item[cfg.nameKey] + '</div>' +
                (subText ? '<div class="cp-item-sub">' + subText + '</div>' : '') +
                price;
    ...
    function load(search, extra) {
        ...
        const params = Object.assign({ search: search || '' }, extra || {});
        $.get(cfg.fetchUrl, params, renderGrid).fail(() => {
            grid.innerHTML = '<div class="cp-no-results">Failed to load. Try again.</div>';
        });
    }
...
const productPicker = new GridPicker({
    gridId:         'product-grid',
    selectedListId: 'product-selected-list',
    hiddenInputsId: 'product-hidden-inputs',
    badgeId:        'product-count-badge',
    countSpanId:    'product-selected-count',
    inputName:      'product_ids',
    fetchUrl:       SEARCH_PRODUCTS_URL,
    nameKey:        'name',
    subKey:         'restaurant_name',
    imgKey:         'image_full_url',
    fallback:       FALLBACK_FOOD_IMG,
    currency:       '',
});
productPicker.load('', {});

// ── Init restaurant picker ────────────────────────────────────────────────────
const restaurantPicker = new GridPicker({
    gridId:         'restaurant-grid',
    selectedListId: 'restaurant-selected-list',
    hiddenInputsId: 'restaurant-hidden-inputs',
    badgeId:        'restaurant-count-badge',
    countSpanId:    'restaurant-selected-count',
    inputName:      'restaurant_ids',
    fetchUrl:       SEARCH_RESTAURANTS_URL,
    nameKey:        'name',
    subKey:         'address',
    imgKey:         'logo_full_url',
    fallback:       FALLBACK_REST_IMG,
    currency:       '',
});
restaurantPicker.load('', {});

// ── Product search + restaurant filter ───────────────────────────────────────
let productDebounce;
...
$('#custom-page-form').on('submit', function (e) {
    ...
    $.ajax({
        ...
        success: function () {
            toastr.success('Custom page created successfully!');
            setTimeout(() => window.location.href = '{{ route("admin.custom-page.index") }}', 800);
        },
        error: function (xhr) {
            btn.disabled = false; btn.innerHTML = '<i class="tio-save mr-1"></i> Save Page';
